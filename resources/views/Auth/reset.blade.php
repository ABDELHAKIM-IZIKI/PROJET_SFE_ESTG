    ...
    <form class="max-w-sm w-full p-10 bg-white rounded-lg shadow-md" action="{{ route('Auth.role') }}" method="POST">
        ...
        <div class="mb-5">
           <span class="block text-sm text-gray-600 font-medium ">
                Entrez l'adresse e-mail avec laquelle vous vous êtes inscrit précédemment pour recevoir un lien dans votre boîte afin de modifier votre mot de passe
            </span>
        </div>
        ...
        <div class="text-right">
            <button type="submit" class="text-white font-medium rounded-lg text-sm px-5 py-2.5 bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-800">Envoyer le lien</button>
        </div>
    </form>
</div>
@endsection