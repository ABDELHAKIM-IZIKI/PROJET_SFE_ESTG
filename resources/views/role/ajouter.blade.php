...
<body>
 ...
 <!--titre-->
  <div class="mb-5" ><h1 class="text-4xl text-black font-bold no-italic ">Ajouter un nouveau role : </h1><br/></div>
<!--form-->
<div class="h-screen mx-10">
    <form method="POST" action="{{route('role.create')}}" class="max-w-sm mx-auto">
        @csrf
        <div class="mb-5">
          <label for="Nom" class="block mb-2 text-sm font-medium text-black ">Nom de Role</label>
          <input value="{{ $user['nom'] ?? ''}}" type="text" id="Nom" name="nom"  class="shadow-sm text-white  text-sm rounded-lg  block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400  focus:ring-blue-500 focus:border-blue-500 shadow-sm-light" placeholder="se nom"  />
        </div>
   ...
   <div class="mb-5">   
    <button type="submit" class="rounded-lg m-5 h-10 px-4 py-2 w-25 mx-2 bg-blue-600 hover:bg-blue-700  text-white">Ajouter le role</button>
    </div>
    </form>
